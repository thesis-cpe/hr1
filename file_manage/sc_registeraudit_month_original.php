...
include_once('inc/config.php');
	...
	include_once('inc/connect.php'); 
			...
			{/*ติดตรงนี้*/
				...
				{
					$sql = ("INSERT INTO audit_rc_mth (audit_rc_m_imp_dat, audit_rc_m_import_who, audit_rc_m_yt, audit_rc_m_mt, audit_rc_m_activity, audit_rc_m_complete, audit_rc_m_complete_dat, audit_rc_m_progress, audit_rc_m_valid, audit_rc_m_valid_dat, audit_rc_m_problem, fk_n_customer_id) 
						VALUES ('$datein', '$employee', '$year', '$month', '$data_detail[$value]','$data_complete[$value]','$data_complete_date[$value]', '$data_pro[$value]', '$data_valid[$value]', '$data_valid_date[$value]', '$data_prob[$value]', '$cusgen')");
					$result = $conn->query($sql);
					//echo $data_detail[$value]."---".$data_complete[$value]."---".$data_complete_date[$value]."---".$data_pro[$value]."---".$data_valid[$value]."---".$data_valid_date[$value]."---".$data_prob[$value]."<br>";
				}
				/*foreach ($data_chb as $key => $value) 
				{
				...
				/*แบงค์ แก้เพิ่ม*/
					...
					$sqlSelMaxId = "SELECT MAX(audit_rc_m_id) AS max_id FROM audit_rc_mth";
					$querySelMaxId = $conn->query($sqlSelMaxId);
					while($arrMaxId = $querySelMaxId->fetch_array())
					{
						$maxId = $arrMaxId['max_id'];
				...
				if(count($data_chbf)>0)
				{
					...
					if(isset($_FILES["filIn"]))
					{
						...
						foreach ($data_chbf as $key => $value) 
						{
							...
							$random = rand();
							if(move_uploaded_file($thm_name_arrray[$value],"storage/$rowl[fk_customer_tax_id]/audit_month-$random-$date-$cusgen".$name_arrray[$value]))
							/*.แบงค์ เพิ่ม*/
							//if(move_uploaded_file($thm_name_arrray[$value],"storage/$rowl[fk_customer_tax_id]/$cusgen".$name_arrray[$value]))
							{
								$pathtxtPath[$value]  = "storage/$rowl[fk_customer_tax_id]/audit_month-$random-$date-$cusgen".$name_arrray[$value];
								//$pathtxtPath[$value]  = "storage/$rowl[fk_customer_tax_id]/$cusgen".$name_arrray[$value];
								//echo $pathtxtPath;
								//echo "<p>Upload filIn Complete</p>";
								$sqli = ("INSERT INTO file_rc_mth (file_rc_mth_name, file_rc_mth_path, fk_n_customer_id, fk_audit_rc_mth_id) VALUES ('$data_hidf[$value]', '$pathtxtPath[$value]', '$cusgen', '$maxId')");
						...
						if($resulti==TRUE)
						{
							array_push($messages, "บันทึกข้อมูลการรายงาน การจัดทำบัญชีประจำเดือน รหัสงาน: $cusgen เสร็จสิ้น");
						}
						else
						{
							die("<script>
...
<div class="container container-padding">
	<?php
			...
			else
			{
    			...
    			echo('<p class="footer">');
    			echo('<center>');
				echo('<a class="btn btn-blue" href="audit_month.php">เพิ่มรายการใหม่</a>');
				echo ('&nbsp;&nbsp;');
