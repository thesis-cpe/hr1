...
<?php
	...
	{	
		...
		$work_id = $_GET['worker'];
		/*$sqla = ("SELECT * FROM coast_work LEFT OUTER JOIN customer_gen ON coast_work.fk_customer_tax_id=customer_gen.fk_customer_tax_id LEFT OUTER JOIN customer ON customer.customer_tax_id=customer_gen.fk_customer_tax_id
			 WHERE coast_work.fk_employee_worker_id=$_GET[worker] GROUP BY coast_work_id ORDER BY coast_work.fk_n_customer_id DESC");
		...
		if(@$_GET['view']==NULL)
		{
			$sqla = ("SELECT * FROM coast_work LEFT OUTER JOIN customer_gen ON coast_work.fk_n_customer_id=customer_gen.n_customer_id LEFT OUTER JOIN customer ON customer.customer_tax_id=customer_gen.fk_customer_tax_id
			 WHERE coast_work.fk_employee_worker_id=$_GET[worker] AND customer_gen.check_close=0 GROUP BY coast_work_id ORDER BY coast_work.fk_n_customer_id DESC");
			$resulta = $conn->query($sqla);
			$rowa = $resulta->fetch_assoc();
		}
		else
		{
			$sqla = ("SELECT * FROM coast_work LEFT OUTER JOIN customer_gen ON coast_work.fk_n_customer_id=customer_gen.n_customer_id LEFT OUTER JOIN customer ON customer.customer_tax_id=customer_gen.fk_customer_tax_id
			 WHERE coast_work.fk_employee_worker_id=$_GET[worker] AND customer_gen.check_close=0 GROUP BY coast_work_id ORDER BY $_GET[view]");
			$resulta = $conn->query($sqla);
			$rowa = $resulta->fetch_assoc();
		}
?>
							...
							<div class="col-lg-12">
								<div class="panel panel-red">
									<div class="panel-heading">
										<h3 align="right" class="panel-title"><span style="float:left;" class="fa fa-bar-chart-o">&nbsp;ข้อมูลงานบัญชีและงานภาษี&nbsp;&nbsp;&nbsp;</span></h3>
										<!--
										<?php
										...
										<div class="btn-group">
										  ...
										  <ul class="dropdown-menu" role="menu">
										    <li><a href="workuserv2.php?worker=<?php echo $work_id; ?>&view=<?php echo $cusd; ?>">รหัสงานบริษัท มากไปน้อย</a></li>
										    <li><a href="workuserv2.php?worker=<?php echo $work_id; ?>&view=<?php echo $cusa; ?>">รหัสงานบริษัท น้อยไปมาก</a></li>
										    <li><a href="workuserv2.php?worker=<?php echo $work_id; ?>&view=<?php echo $namd; ?>">ชื่อลูกค้า มากไปน้อย</a></li>
										    <li><a href="workuserv2.php?worker=<?php echo $work_id; ?>&view=<?php echo $nama; ?>">ชื่อลูกค้า น้อยไปมาก</a></li>
										  </ul>
										</div>-->
									</div>
												...
												<td>
													<?php
														$sqlz = ("SELECT SUM(dr_time_hour) AS hour, SUM(dr_time_minute) AS minute 
															FROM daily_record WHERE fk_n_customer_id='$rowa[fk_n_customer_id]' AND dr_em_id=$_GET[worker]");
														...
														$sqlq = ("SELECT coast_work_hour FROM coast_work WHERE fk_n_customer_id='$rowa[fk_n_customer_id]' AND fk_employee_worker_id=$_GET[worker]");
																						...
																						<tr>
																							...
																							<?php 
																								$sqld = ("SELECT SUM(dr_time_hour) AS hourd, SUM(dr_time_minute) AS minuted 
																									FROM daily_record WHERE dr_detail='$rowb[condition_name]' AND dr_em_id=$_GET[worker] AND fk_n_customer_id='$rowa[fk_n_customer_id]'"); 
																								...
																								$sqle = ("SELECT SUM(dr_record) AS rec FROM daily_record WHERE dr_em_id=$_GET[worker] AND fk_n_customer_id='$rowa[fk_n_customer_id]'");
																							...
																							?>
																							<td><?php echo intval($thourd); ?>&nbsp;ชั่วโมง&nbsp;<?php echo $tminuted; ?>&nbsp;นาที</td>
																							<td>
																								<div class="progress">
