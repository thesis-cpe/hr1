        ...
        if(isset($_FILES["filIn"]))
        {
            ...
            { /*แบงค์เพิ่ม*/
               ...
               // if(move_uploaded_file($thm_name_arrray[$value],"storage/$rowl[fk_customer_tax_id]/$cusgen".$name_arrray[$value]))
				if(move_uploaded_file($thm_name_arrray[$value],"storage/$rowl[fk_customer_tax_id]/tax_year-$date-$random-$cusgen".$name_arrray[$value]))
                {
					$pathtxtPath[$value]  = "storage/$rowl[fk_customer_tax_id]/tax_year-$date-$random-$cusgen".$name_arrray[$value];
                    //$pathtxtPath[$value]  = "storage/$rowl[fk_customer_tax_id]/$cusgen".$name_arrray[$value];
                    //echo $pathtxtPath;
                    //echo "<p>Upload filIn Complete</p>";
                    $sqli = ("INSERT INTO year_tax (yt_import_dat, yt_import_who, yt_year_tax, yt_month_tax, yt_tax_name, yt_filing_dat, yt_payment, yt_payment_dat, yt_receip_no, yt_receip_file, fk_n_customer_id) VALUES ('$dateimp', '$employee', '$year', '$month', '$data_detail[$value]', '$data_in[$value]', '$data_price[$value]', '$data_pay[$value]', '$data_bill[$value]', '$pathtxtPath[$value]', '$cusgen')");
                ...
                {
                    die("<script>
                        alert('ผิดพลาด! : บันทึกข้อมูลการรายงาน การจัดทำภาษีประจำปี ผิดพลาด');
                        history.back();
                        </script>");
                }
            }
                if($resulti==TRUE)
                {
                    array_push($messages, "บันทึกข้อมูลการรายงาน การจัดทำภาษีประจำเปี รหัสงาน: $cusgen เสร็จสิ้น");
                }
                else
                {
                    die("<script>
...
<div class="container container-padding">
    <?php
            ...
            else
            {
                ...
                echo('<p class="footer">');
                echo('<center>');
                echo('<a class="btn btn-blue" href="tax_year.php">เพิ่มรายการใหม่</a>');
                echo ('&nbsp;&nbsp;');
