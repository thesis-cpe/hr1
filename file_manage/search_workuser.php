...
$value=$_POST["value"];
	...
	if($value==""){
		$sql = ("SELECT * FROM coast_work LEFT OUTER JOIN customer_gen ON coast_work.fk_n_customer_id=customer_gen.n_customer_id LEFT OUTER JOIN customer ON customer.customer_tax_id=customer_gen.fk_customer_tax_id
		...
		$sql = ("SELECT * FROM coast_work LEFT OUTER JOIN customer_gen ON coast_work.fk_n_customer_id=customer_gen.n_customer_id LEFT OUTER JOIN customer ON customer.customer_tax_id=customer_gen.fk_customer_tax_id
			 WHERE customer.customer_company LIKE '%".$value."%' || customer_gen.n_customer_id LIKE '%".$value."%' AND coast_work.fk_employee_worker_id=$_SESSION[login] AND customer_gen.check_close=0 GROUP BY coast_work_id ");
	}
