...
/*CASE การเลือก*/
 ...
 if($txtCus != "" && $txtName == "" && $txtYear == "")// 1 0 0
 {
     ...
     //$selWorkReport = "SELECT * FROM `coast_work` WHERE `fk_n_customer_id` = '$txtCus' GROUP BY(`fk_employee_worker_id`)";
     $selWorkReport = "SELECT coast_work_role, employee_name, employee_lname, employee_worker_id, coast_work_bath, coast_work_hour FROM `coast_work` JOIN employee ON coast_work.fk_employee_worker_id = employee.employee_worker_id WHERE `fk_n_customer_id` = '$txtCus' GROUP BY(`fk_employee_worker_id`)";
                                             ...
                                             ?>
                                                    <tr>
                                                      ...
                                                      ?>
                                                        ...
                                                        <td align="center"><?php 
                                                                echo number_format($resultSelDrRec['drRecord']);
...
<?php if($selWorkReport != ""){ ?>
 ...
 <?php
   $quryWorkReport = $conn->query($selWorkReport);
   while($arrWorkReport = $quryWorkReport->fetch_array())
   {   $workReportEmID = $arrWorkReport['employee_worker_id'];
   
   //ค่าจาก dr_record รายบุคคล
    $sqlWorkReportSelDr = "SELECT SUM(dr_record) AS sumDrRec,SUM(dr_time_hour) AS sumDrHour, SUM(dr_time_minute) AS sumDrminute FROM `daily_record` WHERE dr_em_id = '$workReportEmID' AND fk_n_customer_id = '$txtCus'";
    $queryWorkReportSelDr = $conn->query($sqlWorkReportSelDr);
    $resultWorkReportSelDr = $queryWorkReportSelDr->fetch_array();
 ?>                                                   
                                                    <tr>
                                                    <!--ชื่อ นามสุล-->    
                                                        <td><?php echo $arrWorkReport['employee_name'];?>&nbsp;&nbsp;<?php echo $arrWorkReport['employee_lname'];?></td>
                                                        
                                                    <!--ตำแหน่ง-->  
                                                        <td align="center"><?php 
                                                                if($arrWorkReport['coast_work_role'] == 0)
                                                                {
                                                                    echo "ผู้ทำบัญชี";
                                                                }
                                                                elseif($arrWorkReport['coast_work_role'] == 1)
                                                                {
                                                                    echo "ผู้ปฎิบัติงาน";
                                                                }
                                                    ...
                                                    <!--ชั่วโมงประมาณการ-->  
                                                        <td align="center"><?php echo $arrWorkReport['coast_work_hour'];?></td>
    
                                                        
                                                     <!--ชั่วโมงใช้ไป--> 
 <?php //$resultWorkReportSelDr['sumDrHour'];
       //$resultWorkReportSelDr['sumDrminute'];
       $workReportTimeDrtoMinute = calTime($resultWorkReportSelDr['sumDrHour'], $resultWorkReportSelDr['sumDrminute']); //ชัั่โมงใช้ไปคำนวณเป็นนาที
       $workReportTimeDrtoMinuteToHour = convMinToHourHour($workReportTimeDrtoMinute); //ชั่วโมงใช้ไปแปลงเป็นชั่วโมง
 ?>   
                                                        <td align="center"><?php echo $workReportTimeDrtoMinuteToHour;  ?></td>
                                                        
                                                     <!--ผลต่างชั่วโมง--> 
    <?php
        $workReportDifTimeMinute  =  ($arrWorkReport['coast_work_hour'] * 60) -  $workReportTimeDrtoMinute; //ลบกันแล้วหน่วยเป็นนาที
        $workReportDifTimeMinuteToHour = convMinToHourHour($workReportDifTimeMinute); //แปลงเป็นชั่งโมง
    ?>
                                                        <td align="center"><?php echo $workReportDifTimeMinuteToHour; ?></td>
                                                     <!--record-->    
                                                     <td align="right"><?php echo number_format($resultWorkReportSelDr['sumDrRec']); ?></td>
                                                        
                                                     <!--ค่าแรงประมาณการ--> 
    <?php
        $workReportIntiialCoast = $arrWorkReport['coast_work_hour'] * $arrWorkReport['coast_work_bath'];
    ?>
                                                     <td align="right"><?php echo number_format($workReportIntiialCoast);?></td>
                                                        
                                                     <!--ค่าแรงใช้จริง-->
    <?php
        $workReportRealCoast = $workReportTimeDrtoMinuteToHour * $arrWorkReport['coast_work_bath'];
    ?>
                                                     <td align="right"><?php echo number_format($workReportRealCoast); ?></td>
                                                        
                                                     <!--ผลต่างค่าแรง--> 
    <?php
        $workReportDifCoast = $workReportIntiialCoast - $workReportRealCoast;
    ?>
                                                     <td align="right"><?php echo number_format($workReportDifCoast);?></td>
                                                    </tr>
