...
$messages = array();
	...
	include_once('inc/connect.php');
		...
		$sql = ("DELETE FROM conditions WHERE n_customer_id=$cusnew");
		...
		if($result==TRUE)
		{
			...
			if($result==TRUE)
			{
				...
				$sqlx = ("UPDATE work_details SET work_start='$datestart', work_end='$dateend', work_receip_dat='$workstart', work_coast_choice='$choice', work_coast_bath='$salary', work_period='$timework' WHERE  fk_n_customer_id=$cusnew");
				$resultx = $conn->query($sqlx);
			}
			else
			{
				die("<script>
					alert('ผิดพลาด! : อัพเดตข้อมูล เงื่อนไขการทำบัญชี ผิดพลาด');
					history.back();
			 		</script>");
			}
				$pathtxtPathcon=NULL;
				...
				{
					die("<script>
						alert('ผิดพลาด! : อัพเดตข้อมูล ต้นทุนงาน ผิดพลาด');
						history.back();
			 			</script>");
				}
					if($resulty==TRUE) 
					...
					{
						die("<script>
							alert('ผิดพลาด! : อัพเดตข้อมูล ใบเสนอราคา ผิดพลาด');
							history.back();
			 				</script>");
					}
						if($resultz==TRUE)
						{
							array_push($messages, "รหัสงาน $cusnew อัพเดตเสร็จสิ้น");
						}
						else
						{
							die("<script>
							alert('ผิดพลาด! : อัพเดตข้อมูล สัญญาว่าจ้าง ผิดพลาด');
							history.back();
			 				</script>");
						}
		}
...
<div class="container container-padding">
	<?php
			...
			else
			{
    			...
    			echo('<p class="footer">');
    			echo('<center>');
				echo('<a class="btn btn-blue" href="workdata.php">เพิ่มรายการใหม่</a>');
				echo ('&nbsp;&nbsp;');
